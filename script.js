...
const faqData = [
  {
    keywords: ["発送", "配送", "梱包", "発送方法", "配送方法", "発送手順", "届け"],
    answer: "発送関連です。\n・発送タブでお届け日マップを確認\n・発送のやり方.pdf / 発送連絡手順.pdf を確認\n・梱包や配送案内が必要なときは発送カテゴリを見てください"
  },
  {
    keywords: ["圧着", "ミス", "失敗", "剥がれ", "再圧着", "dtf", "やり直し"],
    answer: "圧着関連です。\n・圧着カテゴリのマニュアルを確認\n・DTF機トラブルは『DTF機が壊れたら.pdf』を参照\n・判断に迷う場合は上司相談にしてください"
  },
  {
    keywords: ["サイズ", "大きさ", "寸法", "ボディサイズ"],
    answer: "サイズ確認は『ボディサイズ』タブを見てください。\n昇華と圧着の表を分けて載せています。"
  },
  {
    keywords: ["マスト", "今月", "must", "予定", "月"],
    answer: "マスト確認は『マスト』タブです。\n月ごとのアコーディオンを開いて最新画像を確認してください。"
  },
  {
    keywords: ["クレーム", "電話", "謝罪", "対応"],
    answer: "クレーム・電話対応は『電話』カテゴリのマニュアル確認がおすすめです。\nクレーム対応マニュアル.pdf も開けます。"
  },
  {
...
function resetPrices() {
  costPriceEl.textContent = "¥0";
  salePriceEl.textContent = "¥0";
  minPriceEl.textContent = "¥0";
  detailBodyEl.textContent = "0円";
  detailPrintEl.textContent = "0円";
  detailShippingEl.textContent = "0円";
}
...
function calculatePrice() {
  ...
  costPriceEl.textContent = `¥${formatNumber(costPerPiece)}`;
  salePriceEl.textContent = `¥${formatNumber(salePrice)}`;
  minPriceEl.textContent = `¥${formatNumber(minPrice)}`;
  detailBodyEl.textContent = `${formatNumber(bodyPrice)}円`;
  detailPrintEl.textContent = `${formatNumber(printPricePerPiece)}円`;
  detailShippingEl.textContent = `${formatNumber(shipping)}円`;
}
...
input.addEventListener("input", () => {
  ...
  const hits = commandIndex.filter(item =>
